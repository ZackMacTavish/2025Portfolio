...
async function run() {
  try {
    ...
    const targets = files.filter(f => f.startsWith('favicon-') && f.endsWith('.png'));
    if (targets.length === 0) {
      console.log('No favicon PNGs found to flatten.');
      return;
    }
    ...
    console.log(`Processed ${targets.length} favicon PNGs.`);
  } catch (err) {
