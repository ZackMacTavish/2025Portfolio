...
`,Re=r(d.img)`
  ...
  border-radius: ${i=>i.$borderRadius};
`;function F({src:i,alt:o,avif:w,webp:y,aspectRatio:u,parallaxSpeed:v=0,className:C,borderRadius:k="12px",containerBorderRadius:V,backgroundColor:h,imagePaddingBlock:p,objectFit:M="cover",objectPosition:L="center",imageScale:H=1,disableRevealAnimation:G=!1,loading:O="lazy",decoding:b="async",onLoad:N}){const W=m.useRef(null),D=be(),[j,$]=m.useState(()=>typeof window<"u"&&window.innerWidth<768),l=ye(0);m.useEffect(()=>{if(typeof window>"u")return;const a=()=>{$(window.innerWidth<768)};return a(),window.addEventListener("resize",a),()=>window.removeEventListener("resize",a)},[]);const x=Math.max(0,v)*60,E=v>0&&!D&&!j,I=!!u,B=V??k,J=Z(i),U=Z(w),t=Z(y);return m.useEffect(()=>{if(!E){l.set(0);return}const a=()=>{const n=W.current;if(!n)return;const s=n.getBoundingClientRect(),c=window.innerHeight,T=c+s.height,g=T>0?(c-s.top)/T:.5,R=Math.min(1,Math.max(0,g)),P=-x+R*x*2;l.set(P)};return a(),window.addEventListener("scroll",a,{passive:!0}),window.addEventListener("resize",a),()=>{window.removeEventListener("scroll",a),window.removeEventListener("resize",a)}},[x,l,E]),e.jsx(Ie,{ref:W,className:C,$aspectRatio:u,$borderRadius:B,$backgroundColor:h,...G?{}:{initial:{opacity:0,y:24},whileInView:{opacity:1,y:0},viewport:{once:!0,amount:.2},transition:{duration:.9,ease:[.25,.1,.25,1]}},children:e.jsx(ke,{$imagePaddingBlock:p,$fillContainer:I,$borderRadius:k,children:e.jsxs(Ce,{$fillContainer:I,$borderRadius:k,children:[U&&e.jsx("source",{srcSet:U,type:"image/avif"}),t&&e.jsx("source",{srcSet:t,type:"image/webp"}),e.jsx(Re,{src:J,alt:o,loading:O,decoding:b,onLoad:N,style:{y:E?l:0,scale:H},$objectFit:M,$objectPosition:L,$hasParallax:E,$fillContainer:I,$borderRadius:k})]})})})}const Pe=r.div`
  width: 100%;
...
`,Fe=r.button`
  ...
  transition: all 0.3s;
`;function ie({images:i,autoPlay:o=!1,intervalMs:w=4e3,showControls:y=!0,showDots:u=!0}){m.useEffect(()=>{if(!O.current)return;let l=!1;try{O.current.focus({preventScroll:!0}),l=!0}catch{}},[]);const[v,C]=m.useState(0),[k,V]=m.useState(0),[h,p]=m.useState(!0),[M,L]=m.useState(!1),[H,G]=m.useState(!1),O=m.useRef(null),b=i.length,N=l=>{if(!b)return;const x=(l+b)%b;V(v),p(!1),C(x)},W=()=>N(v+1),D=()=>N(v-1);if(m.useEffect(()=>{if(!o||H||b<2)return;const l=setInterval(()=>{C(x=>(x+1)%b)},w);return()=>clearInterval(l)},[o,w,H,b]),m.useEffect(()=>{if(b<2)return;const l=E=>new Promise(I=>{const B=new Image;B.onload=async()=>{if(typeof B.decode=="function")try{await B.decode()}catch{}I()},B.onerror=()=>I(),B.src=E});(async()=>{const E=i.flatMap(I=>[I.avif,I.webp,I.src].filter(B=>!!B));await Promise.all(E.map(I=>l(I)))})()},[i,b]),!b)return null;const j=i[v],$=i[k];return e.jsxs(Pe,{children:[e.jsxs(ge,{ref:O,tabIndex:0,"data-carousel-frame":"true",onMouseEnter:()=>L(!0),onMouseLeave:()=>L(!1),onKeyDown:l=>{l.key==="ArrowRight"&&(l.preventDefault(),G(!0),W()),l.key==="ArrowLeft"&&(l.preventDefault(),G(!0),D())},drag:"x",dragConstraints:{left:0,right:0},onDragEnd:(l,x)=>{x.offset.x>80?D():x.offset.x<-80&&W()},children:[!h&&e.jsx(ae,{style:{zIndex:1},children:e.jsx(ne,{src:$.src,alt:$.alt,avif:$.avif,webp:$.webp,aspectRatio:$.aspectRatio||"3/2",borderRadius:"0",backgroundColor:$.backgroundColor,imagePaddingBlock:$.imagePaddingBlock,objectFit:$.objectFit||"cover",objectPosition:$.objectPosition||"center",disableRevealAnimation:!0,loading:"eager",decoding:"auto"},`prev-${$.src}-${k}`)}),e.jsx(ae,{style:{zIndex:2},children:e.jsx(ne,{src:j.src,alt:j.alt,avif:j.avif,webp:j.webp,aspectRatio:j.aspectRatio||"3/2",borderRadius:"0",backgroundColor:j.backgroundColor,imagePaddingBlock:j.imagePaddingBlock,objectFit:j.objectFit||"cover",objectPosition:j.objectPosition||"center",disableRevealAnimation:!0,loading:"eager",decoding:"auto",onLoad:()=>p(!0),style:{position:h?"relative":"absolute"}},`active-${j.src}-${v}`)}),b>1&&y&&e.jsxs(Se,{$visible:M,children:[e.jsx(re,{type:"button",$side:"left","aria-label":"Previous image",onClick:D,children:"‹"}),o&&e.jsx(Be,{type:"button","aria-label":H?"Resume slideshow":"Pause slideshow",onClick:()=>G(l=>!l),children:H?"▶":"❚❚"}),e.jsx(re,{type:"button",$side:"right","aria-label":"Next image",onClick:W,children:"›"})]})]}),b>1&&u&&e.jsx(ze,{children:i.map((l,x)=>e.jsx(Fe,{type:"button",$active:x===v,"aria-label":`Go to slide ${x+1}`,"aria-current":x===v,onClick:()=>N(x)},`${l.src}-${x}`))})]})}const Ve=r.div`
  min-height: 100vh;
  background: white;
`,Ee=r.section`
  position: relative;
