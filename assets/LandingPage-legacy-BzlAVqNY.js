...
`,qt=a.div`
  ...
  &::before {
    top: 20%;
    left: 10%;
    animation: ${St} 35s ease-in-out infinite;
  }

  &::after {
    top: 40%;
    left: 65%;
    animation: ${Gt} 40s ease-in-out infinite;
  }
...
`,Pt=e=>i.jsx(Fe,{desktop:ue,desktopAvif:we,desktopWebp:be,mobile600:xe,mobile900:ve,mobile600Avif:ye,mobile900Avif:ke,mobile600Webp:je,mobile900Webp:Ce,alt:"Zack MacTavish portrait",loading:"eager",decoding:"async",fetchPriority:"high",className:"about-picture-img",style:e.style}),Nt=a.div`
  display: flex;
  position: relative;
  z-index: 1;
...
`,ei=a.div`
  animation: ${Qt} 1.5s infinite ease-in-out; /* animate only Y so X-centering stays intact */
...
`,li=a.div.attrs({"data-cursor":"link",role:"link"})`
  display: flex;
  flex-direction: column;
  gap: 6px;
...
`,ui=a.img.attrs({loading:"lazy",decoding:"async"})`
  width: 100%;
  height: auto;
